"""
Modified version of the script associated with the following publication:
https://www.nature.com/articles/s41467-022-31210-w
...
class GRID(MEASURE):
    ...
    def __init__(self, origin=[0., 0., 0.], length=[1., 1., 1.], spacing=[0.2, 0.2, 0.2]):
    ...
    def map2grid(self, points, vdw=False):
        ...
        if vdw:
            ...
            for crd, r in zip(points[::, :3], points[::, 3]):
                ...
                for i in ndx:
                    # Check whether any grid points are out of bounds
                    if not any([c in lim for c, lim in zip(self._grid_points[i], [xlim, ylim, zlim])]):
                        temp_grid_values[i] = 1.0  # KP 2022
    ...
    def _is_mapable(self, r):
        """
        Check whether coordinates r are within 1 spacing of the grids borders
        """
...
class singlegrid:
    ...
    def run(self):
        for frame in self.crd:
            self.grid.map2grid(frame, vdw=self.vdw)
        # put the mapped values in the queue
        self.queue.put(self.grid._grid_values)
    ...
    def run(self, args):
        ...
        for i,f in enumerate(infiles):
            ...
            for st in structure.StructureReader(f):
                print(self._get_vdw_coordinates(st).shape)
                input_coordinates.append(self._get_vdw_coordinates(st))

        input_coordinates = np.array(input_coordinates, dtype=object) # KP 2022
        # Determine grid dimensions

        # Buffer zone around the grid
        concatenatable = [input_coordinates[i] for i in range(len(input_coordinates))] # KP 2022
        input_coordinates_concat = np.concatenate(concatenatable) # KP 2022
        buffer = 3.0 * np.max([1.0, self.args.scale])
        xmin, xmax = (np.min(input_coordinates_concat.T[0].flatten()) - buffer, np.max(input_coordinates_concat.T[0].flatten()) + buffer)
        ymin, ymax = (np.min(input_coordinates_concat.T[1].flatten()) - buffer, np.max(input_coordinates_concat.T[1].flatten()) + buffer)
        zmin, zmax = (np.min(input_coordinates_concat.T[2].flatten()) - buffer, np.max(input_coordinates_concat.T[2].flatten()) + buffer)
        ...
        for inp_coords_arr in input_coordinates:
            # print('the shape of transposed input coordinates are ', input_coordinates.T.shape) # KP 2022

            self.info('Spacing: {}'.format(resolution[0]))
            self.info('Origin: {},{},{}'.format(*origin))
            self.info('Length: {}x{}x{}'.format(*length))
            input_coordinates_dist = self._distribute_work(inp_coords_arr, self.args.nproc)
            self.info('mapping coordinates to grid using {} workers'.format(args.nproc))
            queue = Queue() # KP 2022
            singlegrid(input_coordinates_dist, queue, origin, length, resolution, vdw=True).run() # KP 2022
            grid._grid_values += queue.get() # KP 2022


        print('final max: ') # KP 2022
        print(np.max(grid._grid_values)) # KP 2022
        # Normalize the grid by the total number of structures used
        grid.normalize(f=total_st)
        print('normalised max: ') # KP 2022
        print(np.max(grid._grid_values)) # KP 2022
        ...
        grid_points_dict = {}
        for i, (crd, p) in enumerate(zip(grid._grid_points, grid._grid_values)):
            xyz = tuple(np.uint(np.around((crd - grid._origin) / grid._spacing)))
            grid_points_dict[xyz] = p
        ...
        if out_type == 'vis':
            ...
            np.savetxt(outfile, np.hstack((grid._grid_points, grid._grid_values[:, np.newaxis])), header='x,y,z,d', delimiter=',')
