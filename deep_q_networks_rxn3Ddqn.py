...
class DeepQNetwork(object):
  ...
  """
    ...
    def _build_single_q_network(self, observations, head, state_t, state_tp1,
                                done_mask, reward_t, error_weight):  # same for new action space
    ...
    def _build_input_placeholder(self):  # same for new action space
    ...
    def _build_graph(self):  # same for new action space
    ...
    def _build_training_ops(self):  # same for new action space
    ...
    def _build_summary_ops(self):  # same for new action space
    ...
    def log_result(self, smiles, reward):  # same for new action space
    ...
    """
        ...
        print('Epsilon greedy value is ', self.epsilon)

        if stochastic and np.random.uniform() < self.epsilon:
            print('The action was randomly chosen!')
            return np.random.randint(0, observations.shape[0])
        else:
            print('The action was the current optimal!')
            return self._run_action_op(observations, head)

    def train(self, states, rewards, next_states, done, weight, summary=True):  # same for new action space
...
class MultiObjectiveDeepQNetwork(DeepQNetwork):
  ...
  """
    ...
    def __init__(self, objective_weight, **kwargs):  # same for new action space
        """Creates the model function.
    Args:
...
def multi_layer_model(inputs, hparams, reuse=None):  # same for new action space
