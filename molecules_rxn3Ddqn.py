...
def smiles_to_clean_mol(smiles):
    ...
    try:
        ...
        mol = None # KP 2022, defining it in case exception is thrown
    return mol
...
def _bond_addition(state, atoms_with_free_valence, allowed_ring_sizes,
  ...
  """
    ...
    if FLAGS.smarts_mask != 'False':
        core_to_keep_patt = Chem.MolFromSmarts(FLAGS.smarts_mask)
        core_to_keep = set(state.GetSubstructMatch(core_to_keep_patt))
        if len(core_to_keep) == 0:
            return set()
...
def _bond_removal(state):
  ...
  """
    ...
    print('the length of the atom removal set ', len(bond_removal))
    return bond_removal
...
class Molecule(object):
    ...
    """
        ...
        self._valid_actions = self.get_valid_actions(force_rebuild=True)
        self._counter += 1
        reward = self._reward(hparams, episode, steps_left)
        result = Result(
            state=self._state,
            reward=reward,
