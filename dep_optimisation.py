...
class MultiObjectiveRewardMolecule(molecules_mdp.Molecule):
  ...
  """
    ...
    """
        ...
        if FLAGS.cache == 'True':  # with cache An
            InChI_key = Chem.MolToInchiKey(mol)
            if InChI_key in self.cache.keys():
                similarity_score,docking_score, envelope_penalty, sucos, interactions_penalty, total_number_of_interactions,qed, logp, ligand_file_name, reward, reward_discount = self.cache[InChI_key]
                ...
                docking_score, envelope_penalty, sucos, interactions_penalty,total_number_of_interactions,qed, logp, ligand_file_name = run_dqn.run_docking(self, episode, hparams, mol, steps_left, similarity_score)
                num_heavy_atoms = float(Chem.Mol.GetNumHeavyAtoms(mol))
                reward = interactions_penalty / total_number_of_interactions
                discount = self._discount_factor ** (self.max_steps - self._counter)
                reward_discount = reward * discount
                self.cache[InChI_key] = (similarity_score, docking_score, envelope_penalty, sucos, interactions_penalty,total_number_of_interactions,qed, logp, ligand_file_name, reward, reward_discount)
        else:  # without cache
            similarity_score = self.get_similarity(self._state)
            #docking_score, envelope_penalty,sucos, interactions_penalty, end_of_episode, qed, logp, ligand_file_name = run_dqn.run_docking(self, episode, hparams, mol, steps_left, similarity_score)
            docking_score, envelope_penalty, sucos, interactions_penalty, total_number_of_interactions,qed, logp, ligand_file_name = run_dqn.run_docking(self, episode, hparams, mol, steps_left, similarity_score)
            num_heavy_atoms = float(Chem.Mol.GetNumHeavyAtoms(mol))
            reward = interactions_penalty / total_number_of_interactions
            discount = self._discount_factor ** (self.max_steps - self._counter)
            reward_discount = reward * discount
        print('Episode: ' + str(episode) + ' steps left ' + str(steps_left) +
              ' the docking score is ' + str(docking_score) + ' the penalty is ' +
              str(envelope_penalty) + ' sucos is ' + str(sucos) + ' the interactions penalty is ' + str(interactions_penalty) +
              ' interactions_penalty ' + str(interactions_penalty) + ' total num interactions ' + str(total_number_of_interactions) +
              ' the reward is ' + str(reward) + ' the discounted reward is ' + str(reward_discount) +
              ' the smiles ' + self._state + ' the qed is ' + str(qed) + ' the logp is ' + str(logp) + ' ligand file name is ' + ligand_file_name)
